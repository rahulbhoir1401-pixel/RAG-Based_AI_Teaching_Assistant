...
n=5 #we are merging n chunks here

for filename in os.listdir("2.jsons_chunks"):
    if filename.endswith(".json"):
        file_path = os.path.join("2.jsons_chunks",filename)
        with open(file_path, "r",encoding="utf-8") as f:
            data = json.load(f)
            print(data)
            new_chunks = []
            num_chunks = len(data["chunks"])
            num_groups = math.ceil(num_chunks/n) #round off chunk number like instead to 3.2 give 4
